...
def main():
    ...
    if(d['status'] == 200):     #当返回状态码为200，输出天气状况
        ...
        format_time = trans_format(update_time, '%Y-%m-%d %H:%M:%S', '%H:%M:%S')
        cpurl = 'https://api.day.app/push'
        ...
        data = {
            'body': p,
            'device_key': 'xQvkTS3VuhXsNBP74GVX78',
            'title': f'今日推送-{time}'
        }

        requests.post(cpurl,data)         #把天气数据转换成UTF-8格式，不然要报错。
    else:
